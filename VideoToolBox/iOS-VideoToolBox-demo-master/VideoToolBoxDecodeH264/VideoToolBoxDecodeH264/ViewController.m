...
//
@property (nonatomic, assign) BOOL openGLOrSampleBufferDisplayLayer_Flag;
@property (nonatomic,strong)AAPLEAGLLayer *playLayer;  //解码后播放layer
@property (nonatomic, strong) WTPlayView *playView;
...
- (void)viewDidLoad {
    ...
    //OpenGL渲染 或 AVSampleBufferDisplayLayer渲染
    _openGLOrSampleBufferDisplayLayer_Flag = NO; //
}
...
- (void)initPreviewLayer{
    ...
    [self.previewLayer setFrame:CGRectMake(40, 100,width,height)];
...
- (void)initPlayLayer{
    CGFloat height = (self.view.frame.size.height - 100)/2.0 - 20;
    CGFloat width = self.view.frame.size.width - 100;
    if (_openGLOrSampleBufferDisplayLayer_Flag) {
        //OpenGL渲染
        self.playLayer = [[AAPLEAGLLayer alloc] initWithFrame:CGRectMake(50, (self.view.frame.size.height - 100)/2.0 + 100,width,height)];
        self.playLayer.backgroundColor = [UIColor whiteColor].CGColor;
        [self.view.layer addSublayer:self.playLayer];
    }else {
        
        //AVSampleBufferDisplayLayer
        self.playView = [[WTPlayView alloc]initWithFrame:CGRectMake(50, (self.view.frame.size.height - 100)/2.0 + 100,width,height)];
...
- (void)gotDecodedFrame:(CVImageBufferRef)imageBuffer{
    if(imageBuffer)
    {
        ...
        if (_openGLOrSampleBufferDisplayLayer_Flag) {
            ...
            [self.playView showWityPixelBuffer:pixelBuffer];
...
- (UIButton *)startBtn{
    if (!_startBtn) {
        _startBtn = [[UIButton alloc]initWithFrame:CGRectMake(220, 30, 100, 40)];
...
- (UILabel *)titleLabel{
    if (!_titleLabel) {
        _titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(50, 35, 150, 30)];
...
- (UILabel *)firstLabel{
    if (!_firstLabel) {
        _firstLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 70, 100, 30)];
...
- (UILabel *)secondLabel{
    if (!_secondLabel) {
        _secondLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 360, 100, 30)];
